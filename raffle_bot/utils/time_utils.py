"""
time_utils.py — timezone-aware helpers for Betine Bot.

All "current time" checks are done in the server's configured timezone
(default: Europe/Istanbul) so deposit time validation is always correct.
"""
...
def format_date_display(d: date) -> str:
    """27.06.2026 (Bugün) style."""
    today = today_local()
    suffix = " (Bugün)" if d == today else (
        " (Dün)" if d == today - timedelta(days=1) else ""
    )
